                ...
                <table border="1">
                    ...
                    <tbody>
                        <c:forEach var="userVO" items="${users}">
                            <tr>
                                <td><a href="user?searchName=${userVO.user.username}&tenantId=${userVO.user.tenantId}">${userVO.user.username} <c:if test="${userVO.user.tenantId != null}">(${userVO.user.tenantId})</c:if></a></td>
                                <td>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <jsp:include page="permissionOptions.jsp">
                                                ...
                                                <jsp:param name="name" value="${userVO.user.username}|${userVO.user.tenantId}"/>
                                            </jsp:include>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                ...
                <table border="1">
                    ...
                    <tbody>
                        <c:forEach var="roleVO" items="${roles}">
                            <tr>
                                <td><a href="role?searchName=${roleVO.role.roleName}&tenantId=${roleVO.role.tenantId}">${roleVO.role.roleName} <c:if test="${roleVO.role.tenantId != null}">(${roleVO.role.tenantId})</c:if></a></td>
                                <td>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            ...
                                            <jsp:include page="permissionOptions.jsp">
                                                ...
                                                <jsp:param name="name" value="${roleVO.role.roleName}|${roleVO.role.tenantId}"/>
                                            </jsp:include>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
