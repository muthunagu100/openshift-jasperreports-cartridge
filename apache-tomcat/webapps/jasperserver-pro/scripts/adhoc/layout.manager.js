...
/**
 * @version: $Id: layout.manager.js 6613 2014-07-18 09:12:59Z kklein $
 */
...
(function($, P, _, exports) {
    ...
    LayoutManager.fn = LayoutManager.prototype = {
        ...
        getTemplate : function() {
            if (this.template === null) {
                this.template = lm.compileTemplate("#" + this.mode + "LayoutManagerTemplate", {variable : "data"});
            }
        ...
        render : function(state, isReRender) {
            ...
            _.each(this.axes, function(axis) {
                ...
                $(axis.element).html((this.getTemplate())(axis.data));
        ...
        onContextMenu : function(event) {
            ...
            this.element.trigger('lm:contextMenu', _.extend(event.memo, {
                extra : {
                    ...
                    dimensionId : levelEl.attr('data-dimension'),
        ...
        onRemove : function(e) {
            ...
            this.element.trigger('lm:removeItem', {axis : axis.name, index : pos,
                item : {level : item.attr('data-level'), dimensionId : dim, isMeasure : this.measuresGroupId === dim}});
        },
        ...
        add : function(nodes, pos, axisName) {
            ...
            if (_.isEmpty(nodes)) {
                ...
                this.element.trigger('lm:addItem', {axis : axisName, dimensionId : extra.dimensionId, level : extra.id, index : pos, hierarchyName : extra.hierarchyName});
            } else {
     ...
     */
    LayoutManager.compileTemplate = function(templateId, options) {
        return _.template($(templateId).html(), null, _.defaults(options, {
            evaluate:/\{\{([\s\S]+?)\}\}/g,
            interpolate:/\{\{=([\s\S]+?)\}\}/g,
            escape:/\{\{-([\s\S]+?)\}\}/g
        }));
    ...
    modeExtensionMap = {
        table : {
            ...
            filter : function(nodes, axis) {
                nodes = lm.basicFilter(nodes);
                // Don't filter out duplicated items in column axis
                if (axis.name === 'column') return nodes;

                return lm.removeDuplicatesFilter(nodes, axis.data, {id : 'name'});
            },
                        ...
                        if (!_.isEmpty(jsonLevel.members)) {
                            ...
                            for (var memberIndex = 0; memberIndex < jsonLevel.members.length; memberIndex++) {
                                ...
                                label = (jsonMember.fieldDisplay ? jsonMember.fieldDisplay : jsonMember.name) +
                                    (jsonMember.aggregateColumnName && jsonMember['function'] !== "Sum" && jsonMember.functionOrDefault?
