		...
		{{ if (member.isGroupMember) { }}
			...
			<tr id="{{=member.id}}"
				...
				{{ _.each(member.groupSummaryRow.members, function(footerCell, index) { }}
					{{ if (index === 0) { }}
					<td class="value">{{=escapeTags(formatString(totalsMessage, member.formattedValue))}}</td>
					{{ } else { }}
					<td class="value {{ footerCell.isNumeric ? print('numeric') : print('') }}">{{= escapeTags(footerCell.formattedContent)}}</td>
					{{ } }}
				{{ }); }}
			</tr>
			{{ } else { }}
			<tr class="placeholder member labels">
    				<td class="label" colspan="{{ print(columns.length); }}">
    					<span id="{{=member.id}}" class="{{=member.rowClass}} placeholder member labels" >{{= escapeTags(member.formattedValue)}}</span>
    				</td>
    			</tr>
			{{ } }}
			<!-- END: Group -->
		...
		{{ if (member.isRow && partial.table.showTableDetails) { }}
			...
			<tr class="record">
    				...
    				<td class="value {{ detailCell.isNumeric ? print('numeric') : print(''); }}">
    					<span class="wrap">{{= (!detailCell.formattedContent ? '&nbsp;' : escapeTags(detailCell.formattedContent)) }}</span>
    				</td>
    				{{ }); }}
    			</tr>
			{{ } }}
	...
	<tr id="grandSummaryRow" class="grand columnSummaries stripe">
    		{{ _.each(partial.table.summaryRow.members, function(summaryCell, index) { }}
    			...
    			<td id="grandSummaryCell_{{=index}}"
    				...
    				data-summaryIndex="{{=index}}"
    				data-fieldName="{{=partial.table.columns[index].fieldName}}"
    				data-name="{{=partial.table.columns[index].summaryFunction}}">{{= escapeTags(summaryCell.formattedContent)}}
    			</td>
    			{{ } }}
    		{{ }); }}
    	</tr>
